#include "../Core/LCD_Private.h"      /* private modul definitions & config */
#include "../Core/GUI_Private.h"
#include "../Core/GUIDebug.h"
#include "CH-K-Lib/Libraries/devices/inc/ili9320.h"
...
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex) {
  ...
  {
    ili9320_write_pixel(xPhys, yPhys, PixelIndex);
  }
}
...
unsigned int LCD_L0_GetPixelIndex(int x, int y) {
  ...
  {
    PixelIndex = ili9320_read_pixel(xPhys, yPhys);
  }
  return PixelIndex;
}
...
{
  LCD_PIXELINDEX PixelIndex = WHITE;
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR)
    ili9320_hline(x0, x1, y, LCD_NUM_COLORS - PixelIndex - 1);
  else
    ili9320_hline(x0, x1, y, PixelIndex);
}
...
void LCD_L0_DrawVLine  (int x, int y0,  int y1)
{
  LCD_PIXELINDEX PixelIndex = WHITE;
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR)
    ili9320_vline(y0, y1, x, LCD_NUM_COLORS - PixelIndex - 1);
  else
    ili9320_vline(y0, y1, x, PixelIndex);
}
